...
class ApiExceptionHandlerTest {
  ...
  @Test
  void createsInventoryNotFoundProblem() {
    final var problem =
        handler.inventoryNotFound(new InventoryNotFoundException(UUID.randomUUID()));
    assertThat(problem.getStatus()).isEqualTo(HttpStatus.NOT_FOUND.value());
  }

  @Test
  void createsBadRequestProblemWithFieldErrors() {
    BindingResult bindingResult = Mockito.mock(BindingResult.class);
    Mockito.when(bindingResult.getFieldErrors())
        .thenReturn(java.util.List.of(new FieldError("request", "name", "must not be blank")));
